    ...
    <x-slot name="header">
        <div class="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
            <div>
                ...
                <h2 class="mt-2 text-3xl font-semibold tracking-tight text-white">Caixa diario</h2>
    ...
    <div class="space-y-6">
        ...
        <section class="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
            <article class="sf-card p-5">
                ...
                <p class="mt-3 text-2xl font-semibold text-white">{{ $register ? ($register->closed_at ? 'Fechado' : 'Aberto') : 'Nao aberto' }}</p>
            </article>
            <article class="sf-card p-5">
            ...
            <article class="sf-card p-5">
                <p class="text-xs font-semibold uppercase tracking-[0.18em] text-[#d4af37]">Entradas - saidas</p>
        ...
        <section class="grid gap-6 xl:grid-cols-[360px_minmax(0,1fr)]">
            <aside class="sf-card p-5">
                @if (! $register)
                    ...
                    <form method="POST" action="{{ route('finance.cash.open') }}" class="mt-5 space-y-4">
                        ...
                        <div>
                            ...
                            <x-text-input id="opening_amount" name="opening_amount" type="number" min="0" step="0.01" class="mt-2 block w-full" :value="old('opening_amount', '0.00')" required />
                        </div>
                        <div>
                ...
                @else
                    ...
                    <dl class="mt-5 space-y-3">
                        ...
                        <div class="flex items-center justify-between gap-4">
                            <dt class="text-sm text-[#c7d2e3]">Saidas</dt>
                    ...
                    @if (! $register->closed_at && auth()->user()->isAdmin())
                        <form method="POST" action="{{ route('finance.cash.close') }}" class="mt-6 space-y-4">
                            ...
                            <div>
                                ...
                                <x-text-input id="closing_amount" name="closing_amount" type="number" min="0" step="0.01" class="mt-2 block w-full" :value="old('closing_amount', number_format($register->expectedBalance(), 2, '.', ''))" />
                            </div>
                            <div>
            ...
            <section class="sf-card overflow-hidden">
                <div class="border-b border-white/10 px-6 py-5">
                    ...
                    <p class="mt-1 text-sm text-[#c7d2e3]">Pagamentos de servicos, vendas de produtos e acertos saem daqui.</p>
                </div>
                @if (! $register || $register->movements->isEmpty())
                    <div class="px-6 py-10 text-sm text-[#c7d2e3]">Nenhuma movimentacao registrada neste dia.</div>
                @else
                        ...
                        <table class="min-w-full divide-y divide-white/10">
                            <thead class="bg-[#132746]">
                                <tr>
                                    ...
                                    <th class="px-6 py-3 text-left text-xs font-semibold uppercase tracking-[0.18em] text-[#c7d2e3]">Descricao</th>
